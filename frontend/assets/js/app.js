...
function _initSwipeGesture() {
    ...
    document.addEventListener('touchend', (e) => {
        ...
        const target = e.target.closest('.table-responsive-custom, .chart-container');
...
function buildChart(canvasId, label, color) {
    ...
    return new Chart(ctx, {
        type: 'line',
        data: { labels: [], datasets: [{ label, data: [], borderColor: color.border, backgroundColor: color.bg, borderWidth: 2, pointRadius: 3, pointHoverRadius: 5, tension: 0.4, fill: true }] },
        options: {
            responsive: true, maintainAspectRatio: false, animation: { duration: 400 },
            interaction: { intersect: false, mode: 'index' },
            plugins: {
                legend: { display: false },
                tooltip: { backgroundColor: state.darkMode ? '#1e293b' : '#fff', titleColor: state.darkMode ? '#f1f5f9' : '#0f172a', bodyColor: state.darkMode ? '#94a3b8' : '#64748b', borderColor: state.darkMode ? '#334155' : '#e2e8f0', borderWidth: 1, padding: 10, cornerRadius: 8 }
            },
            scales: {
                x: { grid: { color: gridColor }, ticks: { color: textColor, maxTicksLimit: 6, font: { family:'JetBrains Mono', size:10 } } },
...
function initCharts() {
    ...
    state.charts.hum   = buildChart('chartHum',   'Kelembaban (%)', CHART_COLORS.hum);
    state.charts.soil  = buildChart('chartSoil',  'Kelembaban Tanah (%)', CHART_COLORS.soil);
    state.charts.water = buildChart('chartWater', 'Level Air (%)', CHART_COLORS.water);
}

function updateCharts(rows) {
    if (!rows?.length) return;
    const labels = rows.map(r => formatTime(r.created_at));
    const push = (chart, key) => { if(chart) { chart.data.labels = labels; chart.data.datasets[0].data = rows.map(r => parseFloat(r[key]) || 0); chart.update('none'); } };
...
function updateChartTheme() {
    Object.values(state.charts).forEach(c => {
        ...
        c.options.plugins.tooltip.backgroundColor = state.darkMode ? '#1e293b' : '#fff';
        c.update('none');
    });
}
...
function alertBadgeClass(lvl) { return lvl==='CRITICAL'?'badge-critical':lvl==='WARNING'?'badge-warning':'badge-normal'; }
function alertLabel(lvl) { return lvl==='CRITICAL'?'🔴 Critical':lvl==='WARNING'?'🟡 Warning':'🟢 Normal'; }

function updateStatCards(d) {
    if(!d) return;
    setText('statTemp', d.temperature ? parseFloat(d.temperature).toFixed(1) : '--');
    setHTML('statTempBadge', `<span class="stat-status-badge ${alertBadgeClass(d.temp_alert)}">${alertLabel(d.temp_alert)}</span>`);
    setText('statHum', d.humidity ? parseFloat(d.humidity).toFixed(1) : '--');
    setHTML('statHumBadge', `<span class="stat-status-badge ${alertBadgeClass(d.hum_alert)}">${alertLabel(d.hum_alert)}</span>`);
    setProgress('progHum', d.humidity);
    setText('statSoil', d.soil_percent ?? '--');
    setHTML('statSoilBadge', `<span class="stat-status-badge ${alertBadgeClass(d.soil_alert)}">${alertLabel(d.soil_alert)}</span>`);
    setProgress('progSoil', d.soil_percent);
    setText('statWater', d.water_percent ?? '--');
    setHTML('statWaterBadge', `<span class="stat-status-badge ${alertBadgeClass(d.water_alert)}">${alertLabel(d.water_alert)}</span>`);
    setProgress('progWater', d.water_percent);
    const on = d.pump_status == 1;
    setText('statPump', on ? 'ON' : 'OFF');
    setHTML('statPumpBadge', on ? `<span class="stat-status-badge badge-active"><span class="pump-spinner"></span> Aktif</span>` : `<span class="stat-status-badge badge-inactive">Mati</span>`);
    setText('statPumpLabel', on ? '⚡ Sedang menyiram' : '💤 Pompa standby');
    if(d.created_at) setText('lastUpdate', 'Update: ' + formatDateTime(d.created_at));
}

function setText(id, val) { const n=el(id); if(n) n.textContent=val; }
function setHTML(id, html) { const n=el(id); if(n) n.innerHTML=html; }
function setProgress(id, val) { const bar=el(id); if(bar) bar.style.width = Math.min(100, Math.max(0, parseFloat(val)||0))+'%'; }

function updateAlertPanel(alerts) {
    const wrap = el('alertPanel');
    if(!wrap) return;
    if(!alerts?.length) { wrap.innerHTML='<div class="alert-empty">✅ Tidak ada alert aktif</div>'; return; }
...
function fetchMonitoringHistory() {
    ...
    const params = new URLSearchParams({ action:'history', page, limit, search, sort, dir });
    fetch(`${API}?${params}`).then(r=>r.json()).then(data=>{ if(data.success) renderMonTable(data); }).catch(console.error);
}

function renderMonTable(data) {
    const tbody = el('monTbody');
    if(!tbody) return;
    if(!data.data.length) { tbody.innerHTML=`<tr><td colspan="11" class="text-center py-4 text-secondary">Tidak ada data ditemukan</td></tr>`; }
    else {
    ...
    renderPagination('monPagination', data, (p)=>{ state.monTable.page=p; fetchMonitoringHistory(); });
    setText('monTotal', `${data.total} data`);
}

function fetchAlertHistory() {
    const { page, limit, level } = state.alertTable;
    const params = new URLSearchParams({ action:'alert_history', page, limit, level });
    fetch(`${API}?${params}`).then(r=>r.json()).then(data=>{ if(data.success) renderAlertTable(data); }).catch(console.error);
}

function renderAlertTable(data) {
    const tbody = el('alertTbody');
    if(!tbody) return;
    if(!data.data.length) { tbody.innerHTML=`<tr><td colspan="5" class="text-center py-4 text-secondary">Tidak ada data alert</td></tr>`; }
    else {
        tbody.innerHTML = data.data.map(r => `
            <tr>
                <td class="mono text-secondary">${r.id}</td>
    ...
    renderPagination('alertPagination', data, (p)=>{ state.alertTable.page=p; fetchAlertHistory(); });
...
function renderPagination(containerId, data, onPage) {
    ...
    range.forEach(p => {
        ...
        else btns+=`<button class="pg-btn ${p===page?'active':''}" onclick="goPage('${containerId}',${p})">${p}</button>`;
    });
...
window.goPage = function(containerId, p) {
    if(containerId==='monPagination') { state.monTable.page=p; fetchMonitoringHistory(); }
    else { state.alertTable.page=p; fetchAlertHistory(); }
};
...
function sortMon(col) {
    if(state.monTable.sort===col) state.monTable.dir = state.monTable.dir==='ASC'?'DESC':'ASC';
    else { state.monTable.sort=col; state.monTable.dir='DESC'; }
...
function updateSortIcons() {
    $$('[data-sortcol]').forEach(th=>{ th.classList.remove('sorted'); const ic=th.querySelector('.sort-icon'); if(ic) ic.textContent='⇅'; });
    const active=$( `[data-sortcol="${state.monTable.sort}"]`);
    if(active) { active.classList.add('sorted'); const ic=active.querySelector('.sort-icon'); if(ic) ic.textContent = state.monTable.dir==='ASC'?'↑':'↓'; }
}
...
function fetchRealtime() {
    fetch(`${API}?action=realtime&_=${Date.now()}`).then(r=>{ if(!r.ok) throw new Error(); return r.json(); }).then(data=>{
        ...
        if(dot) { dot.style.background='#4ade80'; setTimeout(()=>{ dot.style.background=''; },400); }
...
function startAutoRefresh() { fetchRealtime(); state.refreshTimer = setInterval(fetchRealtime, TICK); }

/* ════════════════════════════════════════════
   SEARCH DEBOUNCE
   ════════════════════════════════════════════ */
function debounce(fn, ms) { let t; return (...args)=>{ clearTimeout(t); t=setTimeout(()=>fn(...args), ms); }; }
const debouncedSearch = debounce((val)=>{ state.monTable.search=val; state.monTable.page=1; fetchMonitoringHistory(); }, 400);

/* ════════════════════════════════════════════
   DATE UTILS
   ════════════════════════════════════════════ */
function formatTime(ts) { if(!ts) return '--'; const d=new Date(ts); const pad=n=>String(n).padStart(2,'0'); return `${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`; }
function formatDateTime(ts) { if(!ts) return '--'; const d=new Date(ts); const pad=n=>String(n).padStart(2,'0'); return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`; }
...
function bindEvents() {
    ...
    if(searchInp) searchInp.addEventListener('input', e => debouncedSearch(e.target.value.trim()));

    const monLimitSel = el('monLimit');
    if(monLimitSel) monLimitSel.addEventListener('change', e=>{ state.monTable.limit=+e.target.value; state.monTable.page=1; fetchMonitoringHistory(); });

    const alertLimitSel = el('alertLimit');
    if(alertLimitSel) alertLimitSel.addEventListener('change', e=>{ state.alertTable.limit=+e.target.value; state.alertTable.page=1; fetchAlertHistory(); });
